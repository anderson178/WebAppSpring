        ...
        $.get("http://127.0.0.1:8080/getAll")
            .done(function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    $('#names tbody').append('<tr><td>' + data[i]['id'] + '</td><td>' + data[i]['firstName'] + '</td><td>' + data[i]['lastName'] + '</td><td>' + data[i]['middleName'] + '</td><td>' + data[i]['birthDate'] + '</td><td>' + data[i]['comment'] + '</td></tr>')
                }
            })
    });
